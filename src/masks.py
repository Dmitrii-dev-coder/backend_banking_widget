...
def get_mask_account(account_number: str | int) -> str:
    ...
    logger.debug("Функция генерации маски счета запущена")
    ...
    if not account_number_str.isdigit():
        logger.error(f"Номер счета содержит не только цифры: {account_number_str}")
    ...
    # Проверяем длину номера счета
        ...
        logger.error(f"Не совпадает количество цифр в номере счета. Проверьте, пожалуйста, ввод: {account_number_str}")
        raise ValueError("Номер счета должен содержать 20 цифр и никаких других символов")

    # Проверяем на эффект залипания клавиши
    elif len(set(account_number_str)) == 1:
        logger.error("Номер счета содержит только одинаковые цифры (эффект залипания клавиши).")
        raise ValueError("Номер карты содержит только одинаковые цифры (эффект залипания клавиши).")

    # Берем последние 4 цифры и добавляем две звездочки перед ними
    account_mask = f"**{account_number_str[-4:]}"
    logger.debug(f"Маска счета успешно сгенерирована: {account_mask}")
    logger.debug("Функция генерации маски счета завершена")
