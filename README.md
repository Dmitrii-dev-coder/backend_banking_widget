...
В файле `main.py` найдите функцию `checking_masks_functions` (начинается с 14-й строки). Для проверки в коде этой 
функции уже подставлен номер карты в строке 17. Либо вы можете заменить этот номер карты на свой вариант.
...
В файле `main.py` найдите ту же функцию `checking_masks_functions`. Для проверки этой функции в строке 18 уже 
...
В файле `main.py` найдите функцию `checking_widget_functions1` (начинается с 28-й строки). Для проверки в строке 213 
уже подставлен номер счета. Либо вы можете заменить его на свой вариант (примеры вариантов смотри ниже в строках 218-225).
Запустите модуль командой:
```
python main.py
```
...
В файле `main.py` найдите функцию `checking_widget_functions2` (начинается с 37-й строки). Для проверки в строке 214 
...
В файле `main.py` найдите функцию `checking_filter_by_state` (начинается с 46-й строки). Для проверки в строках с 233-й 
по 236-ю уже подставлены данные. Либо вы можете заменить их на свои. Второй аргумент не обязательный (по умолчанию 
...
В файле `main.py` найдите функцию `checking_sort_by_date` (начинается с 53-й строки). Для проверки в строках с 245-й по 248-ю уже подставлены данные для сортировки.
```
python main.py
```
...
В файле `main.py` найдите функцию `checking_filter_by_currency` (начинается с 111-й строки). Функция принимает аргумент - список словарей `transaction` с 64-й по 110-ю строки и второй аргумент - код валюты для фильтрации данных.
```
python main.py
```
...
В файле `main.py` найдите функцию `checking_transaction_descriptions` (начинается с 125-й строки). Функция принимает аргумент - список словарей `transaction` с 62-й по 108-ю строки (задаются в строке 299).
```
python main.py
```
...
В файле `main.py` найдите функцию `checking_card_number_generator` (начинается с 139-й строки). Функция принимает start - начальное значение диапазона и stop - конечное значение диапазона (в строке 317).
```
python main.py
```
Среди результатов отобразится номера карт, которые будут сгенерированы по очереди и отобразятся в консоли с четырнадцатой по восемнадцатую строках:
> 0000 0000 0000 0001
...
11. **Пример использования декоратора `log` с записью в файл:**
В файле `main.py` найдите функцию `checking_log_to_file` (начинается с 154-й строки). Декоратор записывает логи в файл `test_log_example.txt` с временем запуска/завершения работы функции.
```
python main.py
```
Среди результатов отобразится результат выполнения функции с 19-й по 20-ю строку:
> Декоратор log с записью в файл:
> Запуск my_function в 18:12:35.468240

Файл `test_log_example.txt` будет содержать:
> Завершение my_function c результатом: 12 в 12:47:04.127582.

12. **Пример использования декоратора `log` с выводом в консоль:**
В файле `main.py` найдите функцию `checking_log_to_console` (начинается с 167-й строки). Декоратор выводит логи в консоль с временем запуска/завершения работы функции.
```
python main.py
```
Среди результатов отобразится результат выполнения функции с 21-й по 23-ю строку.

Консольный вывод будет содержать:
...
13. **Проверка функции `get_transactions_from_file` из модуля `utils.py`:**
В файле `main.py` найдите функцию `checking_get_transactions_from_file` (начинается с 184-й строки). Для проверки вызовите функцию с путем к файлу (в примере в строке 353 уже есть адрес, его можно заменить на свой).Функция выводит в консоль список словарей транзакций.
```
python main.py
```
Среди результатов в консоли отобразится результат выполнения функции с 24-й по 25-ю строку:
...
14. **Проверка функции `get_sum_of_transaction` из модуля `external_api.py`:**
В файле `main.py` найдите функцию `checking_get_sum_of_transaction`(начинается с 199-й строки). Для проверки вызовите функцию с передачей транзакции (в примере с 377-й строки по 378-й строки). Функция должна вернуть сумму транзакции:
```
python main.py
```
Среди результатов в консоли отобразится результат выполнения функции с 26-й по 27-ю строку:
Консольный вывод будет содержать:
...
Все основные функции из модулей `masks.py`, `widget.py`, `processing.py`, `generators.py`, `units.py` и `external_api.py` покрыты тестами.
