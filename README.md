...
В файле `main.py` найдите функцию `checking_masks_functions` (начинается с 16-й строки). Для проверки в коде этой 
...
В файле `main.py` найдите ту же функцию `checking_masks_functions`. Для проверки этой функции в строке 20 уже 
...
В файле `main.py` найдите функцию `checking_widget_functions1` (начинается с 30-й строки). Для проверки в строке 214 
уже подставлен номер счета. Либо вы можете заменить его на свой вариант (примеры вариантов смотри ниже в строках 219-226).
Запустите модуль командой:
```
python main.py
```
...
В файле `main.py` найдите функцию `checking_widget_functions2` (начинается с 39-й строки). Для проверки в строке 215 
...
В файле `main.py` найдите функцию `checking_filter_by_state` (начинается с 48-й строки). Для проверки в строках с 234-й 
по 237-ю уже подставлены данные. Либо вы можете заменить их на свои. Второй аргумент не обязательный (по умолчанию 
...
В файле `main.py` найдите функцию `checking_sort_by_date` (начинается с 55-й строки). Для проверки в строках с 246-й по 249-ю уже подставлены данные для сортировки.
```
python main.py
```
...
В файле `main.py` найдите функцию `checking_filter_by_currency` (начинается с 113-й строки). Функция принимает аргумент - список словарей `transaction` с 64-й по 110-ю строки и второй аргумент - код валюты для фильтрации данных.
```
python main.py
```
...
В файле `main.py` найдите функцию `checking_transaction_descriptions` (начинается с 127-й строки). Функция принимает аргумент - список словарей `transaction` с 64-й по 110-ю строки (задаются в строке 257).
```
python main.py
```
...
В файле `main.py` найдите функцию `checking_card_number_generator` (начинается с 141-й строки). Функция принимает start - начальное значение диапазона и stop - конечное значение диапазона (в строке 318).
```
python main.py
```
...
В файле `main.py` найдите функцию `checking_log_to_file` (начинается с 156-й строки). Декоратор записывает логи в файл `test_log_example.txt` с временем запуска/завершения работы функции.
```
python main.py
```
Среди результатов отобразится результат выполнения функции с 19-й по 21-ю строку:
> 12
...
В файле `main.py` найдите функцию `checking_log_to_console` (начинается с 170-й строки). Декоратор выводит логи в консоль с временем запуска/завершения работы функции.
```
python main.py
```
Среди результатов отобразится результат выполнения функции с 22-й по 25-ю строку:
> 12
...
В файле `main.py` найдите функцию `checking_get_transactions_from_file` (начинается с 187-й строки). Для проверки вызовите функцию с путем к файлу (в примере в строке 354 уже есть адрес, его можно заменить на свой).Функция выводит в консоль список словарей транзакций.
```
python main.py
```
...
В файле `main.py` найдите функцию `checking_get_sum_of_transaction`(начинается с 201-й строки). Для проверки вызовите функцию с передачей транзакции (в примере с 364-й строки по 378-й строки). Функция должна вернуть сумму транзакции:
```
python main.py
```
Среди результатов в консоли отобразится результат выполнения функции с 28-й по 29-ю строку:
Консольный вывод будет содержать:
