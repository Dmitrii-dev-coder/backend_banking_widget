...
def test_mask_account_card_valid_account_card(account_card, expected):
    assert mask_account_card(account_card) == expected

# Тест 002 Тестирование правильности работы функции с не правильным форматом номера карты или счета.

@pytest.mark.parametrize("account_card", ["Счет 1234567890123456",          #  16 цифр (вставили номер карты)
                                          "Maestro abcde12345",             #  в цифровом номере карты присутствую буквы
                                          ...
                                          "Счет ",                          #  нет цифр номер счета
                                          "Maestro ",                       #  нет цифр номера карты
                                          "Visa Gold 35383033474447895560", #  20 цифр (вставили номер счета)
                                          ""
                                          ])
def test_mask_account_card_not_correct_account_card(account_card):
    # Текст ошибки для человека (читаемый).
    error_text = (
    ...
    # Текст ошибки для машины (обработанный).
...
def test_get_date_with_invalid_data(invalid_data):
    # Текст ошибки для человека (читаемый).
    error_text = (
...
def test_get_date_with_no_date(date_iso_8601):
