...
def delivery_valid_data() -> list[dict]:
    """Фикстура: список словарей с ключами 'state' (EXECUTED и CANCELED)."""
    my_list_dict = [
...
@pytest.fixture
def delivery_invalid_data() -> list[dict]:
    """Фикстура: список словарей без ключа 'state'."""
    my_list_dict = [
...
def test_filter_by_state_valid_data(delivery_valid_data: list[dict]) -> None:
    """Фильтрация по статусу 'CANCELED' возвращает только словари с этим статусом."""
...
# Тест 004: Проверка того, что функция возвращает пустой список если во входных данных отсутствует "state".
def test_filter_by_state_without_state(delivery_invalid_data: list[dict]) -> None:
    """Если в словарях нет ключа 'state', возвращается пустой список."""
...
def test_sort_by_date_valid_data_descending(delivery_valid_data: list[dict]) -> None:
    """Сортировка по умолчанию (descending=True) — от более поздних дат к более ранним."""
...
def test_sort_by_date_valid_data_ascending(delivery_valid_data: list[dict]) -> None:
    """Сортировка по возрастанию (descending=False) — от более ранних дат к более поздним."""
...
def test_sort_by_date_same_date() -> None:
    """Сортировка при одинаковых датах сохраняет стабильный порядок (по убыванию времени)."""
    my_list_dict = [
        {"id": 41428829, "state": "EXECUTED", "date": "2019-07-03T18:35:29.512364"},
...
def test_sort_by_date_invalid_data() -> None:
    """Некорректный формат даты вызывает ValueError с сообщением об ошибке."""
    error_text = (
